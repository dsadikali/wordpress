...
<?php
      if ($adNwOptions['policy'] == 'unknown' && strpos($this->locale, 'en_') !== FALSE) {
?>
...
<textarea cols="50" rows="25" name="adsNowText" style="width: 96%; height: 290px;"><?php echo(stripslashes(htmlspecialchars($adNwOptions['ad_text']))) ?></textarea>
</td>
<td width="450px">
<center>
...
<?php _e('Percentage of ad slots to share [Default: 5%]:', 'easy-adsenser') ; ?> <input style="width:20px;text-align:center;" id="adNwMC" name="adNwMC" value="<?php echo(stripslashes(htmlspecialchars($adNwOptions['mc'])));?>" />%<br />
<label for="adNwAllowPopunder" title="Share your thoughts and comments on this option using the support links below.">
<input type="checkbox" id="adNwAllowPopunder" name="adNwAllowPopunder"  <?php if ($adNwOptions['allow_popunder'] == 'Yes') { echo('checked="checked"'); }?> /> <?php _e('Allow popunders once a day to support the plugin development.', 'easy-adsenser') ; ?></label><br />
<label for="adNwAllowExitJunction" title="[Not implemented yet.] Share your thoughts and comments on this option using the support links below.">
<input type="checkbox" id="adNwAllowExitJunction" name="adNwAllowExitJunction"  <?php if ($adNwOptions['allow_exitjunction'] == 'Yes') { echo('checked="checked"'); }?> /> <?php _e('Allow ExitJunction to donate your search engine bounce traffic.', 'easy-adsenser') ; ?></label><br />
<br style="line-height: 7px;" />
