...
class videoDB{
    ...
    function find_all_galleries($order_by = 'gid', $order_dir = 'ASC', $counter = false, $limit = 0, $start = 0) {      
        ...
        foreach ($galleries as $key => $value) {
            ...
            $galleriess[$key]->title = stripslashes($galleries[$key]->title);
            $galleries[$key]->galdesc  = stripslashes($galleries[$key]->galdesc);
        }

        // if we didn't need to count the videos then stop here
        if ( !$counter )
            return $galleries;
    ...
    function get_gallery($id, $order_by = 'sortorder', $order_dir = 'ASC', $limit = 0, $start = 0) {
        ...
        // Query database
        if( is_numeric($id) )
            $result = $wpdb->get_results( $wpdb->prepare( "SELECT SQL_CALC_FOUND_ROWS tt.*, t.* FROM " . $wpdb->prefix . "cvg_gallery AS t INNER JOIN " . $wpdb->prefix . "cvg_videos AS tt ON t.gid = tt.galleryid WHERE t.gid = %d  ORDER BY tt.{$order_by} {$order_dir} {$limit_by}", $id ), OBJECT_K );
        else
            $result = $wpdb->get_results( $wpdb->prepare( "SELECT SQL_CALC_FOUND_ROWS tt.*, t.* FROM " . $wpdb->prefix . "cvg_gallery AS t INNER JOIN " . $wpdb->prefix . "cvg_videos AS tt ON t.gid = tt.galleryid WHERE t.name = %s ORDER BY tt.{$order_by} {$order_dir} {$limit_by}", $id ), OBJECT_K );
