...
class cvgShowCase_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty( $instance['title'] ) ? __('Showcase') : $instance['title'], $instance, $this->id_base);
		
		$cvg_core = new CvgCore();
		$out = $cvg_core->videoShowGallery($instance['galleryid']);
		if ( !empty( $out ) ) {
			echo $before_widget;
			if ( $title)
...
class cvgSlideShow_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty( $instance['title'] ) ? __('Slideshow') : $instance['title'], $instance, $this->id_base);
		
		$cvg_core = new CvgCore();
		$out = $cvg_core->videoShowGallery($instance['galleryid'], true);
		if ( !empty( $out ) ) {
			echo $before_widget;
			if ( $title)
				echo $before_title . $title . $after_title;
 ...
 * @author Praveen Rajan
 */
function cvgShowCaseWidget($galleryID) {
	$cvg_core = new CvgCore();
	echo $cvg_core->videoShowGallery($galleryID);
}
 ...
 * @author Praveen Rajan
 */
function cvgSlideShowWidget($galleryID) {
	$cvg_core = new CvgCore();
	echo $cvg_core->videoShowGallery($galleryID, true);
}
?>