...
class CategoryImagesII extends CategoryImagesII_Plugin
{
	...
	{
 		$this->error_strings[ 0 ] = __( "Sorry, something went wrong with the image upload. Please try again, or contact the website administrator.", 'category-images-ii' );
 		$this->error_strings[ 1 ] = __( "Sorry, you cannot upload an image file of this type. Allowed image file types are: GIF, JPEG, or PNG.", 'category-images-ii' );
	}
	...
	public function display_images( $cat_ids, $link_images )
	{
	...
	protected function process_upload()
	{
		...
		if ( ! move_uploaded_file( $_FILES[ 'category_images_ii' ][ 'tmp_name' ], $original_file ) ) wp_die( __( "Something went wrong. Could not move the uploaded file.", 'category-images-ii' ) );
...
function ciii_category_archive_image( $args = null )
{
	...
	if ( ! $r[ 'echo' ] )
		return $CategoryImagesII->display_images( $cat_ids );
	echo $CategoryImagesII->display_images( $cat_ids, $r[ 'link_images' ] );
}
