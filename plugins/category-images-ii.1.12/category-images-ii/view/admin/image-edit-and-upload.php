	...
	<table class="form-table" id="category_images_ii">
		<?php if ( $has_image ) { ?>
			<tr class="form-field current-category-image">
				...
				<td>
					...
					<p><input type="submit" class="button-secondary action" name="ciii_delete" value="<?php _e( "Delete This Image", 'category-images-ii' ); ?>" id="Delete" /></p>
				</td>
			</tr>
		<?php } ?>
		<tr class="form-field upload-category-image">
			...
			<td>
				...
				<span class='field-hint'><?php printf( __( "(Will replace any previous image. The maximum uploadable file size is %s.)", 'category-images-ii' ), $max_upload_size ); ?></span>
			</td>
		</tr>
	</table>
<?php } else { // Add category form, at the bottom of the manage categories page. ?>
	<p id="category_images_ii"><?php _e( "To add a category image, edit the category after you've created it.", 'category-images-ii' ); ?></p>
<?php } ?>