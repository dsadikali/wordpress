...
<ul class="gsom-tabs" id="gsom-tabs">
    <li <?php echo $gsom_tab == 'gsom-page-general' ? 'class="gsom-tabs-selected"' : ''?>><span id="gsom-tab-general">General</span></li>
    <li <?php echo $gsom_tab == 'gsom-page-form' ? 'class="gsom-tabs-selected"' : ''?>><span id="gsom-tab-form">Submission Form</span></li>    
...
<p class="submit">
<input id="gsom_tab_inp" type="hidden" name="gsomtab" value="gsom-page-general" />
<input id="gsom_form_submit" type="button" name="submit" value="Update Options &raquo;" />
</p>    
</form>
</div>
<script>        
...
//*/    
    ...
    elseif (stripos($_SERVER['REQUEST_URI'],'confirm-subscription')!== false)
    {
        ...
        new FakePage('confirm-subscription','Confirmation Successful',$msg);        
    }
    elseif (stripos($_SERVER['REQUEST_URI'],'already-confirmed')!== false)
    {
        ...
        $msg = gsom_replaceVars($msg);
        new FakePage('already-confirmed','Already Confirmed',$msg);
    }
    ...
    {      		
        ...
        $msg = gsom_replaceVars($msg);
        new FakePage('already-subscribed','Already Subscribed',$msg);
    }
    elseif (stripos($_SERVER['REQUEST_URI'],'subscription-changed')!== false)
    {        
        new FakePage('subscription-changed','Subscription Changed','<h2>You subscription email was successfully changed.</h2>');
    }
    elseif (stripos($_SERVER['REQUEST_URI'],'unsubscribe-successful')!== false)
    {
        ...
        $msg = gsom_replaceVars($msg);
        new FakePage('unsubscribe-successful','Successfully Unsubscribed',$msg);        
    }
    elseif (stripos($_SERVER['REQUEST_URI'],'bad-email')!== false)
    {
        $msg = get_option('gsom_msg_bad_email_address');
        $msg = gsom_replaceVars($msg);
        new FakePage('bad-email','Bad Email Address',$msg);                        
    }
    elseif (stripos($_SERVER['REQUEST_URI'],'confirmation-failed')!== false)
    {        
        $msg = '<h3>Your email address was not found in our database.</h3>';
        new FakePage('confirmation-failed','Confirmation Failed',$msg);                        
    }
	elseif (stripos($_SERVER['REQUEST_URI'],'not-found')!== false)
	{
        $msg = '<h3>Your email address was not found in our database.</h3>';
        new FakePage('not-found','Email not found',$msg);    
	}
}
